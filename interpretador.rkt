     ...
     (cases expression exp
      ...
      (prim-binary-exp (arg1 prim arg2)                  
       ...
       (if-exp (exp-bool expr-t expr-f)
               (if (and (eval-exp-bool exp-bool env))
...
(define apply-primitive
  (lambda (prim arg1 arg2)
    (cases primitive-binary prim
      ...
      (primitive-subt-oct ()(octal->decimal (-_oct (octal->list arg1) (octal->list arg2) )))
      (primitive-mult-oct () (octal->decimal (*_oct (octal->list arg1) (octal->list arg2))))
...
(define retirar-base (lambda (oct)
                      (substring oct  3 (- (string-length oct) 1))
                       ))

(define comblis (lambda (str inicio)
                  (cond
                    [(equal? str "") '()]
                    [(equal? inicio (- (string-length str) 1)) (cons (string->number (string (string-ref str (- (string-length str) 1) ))) '() )]
                    [ (cons  (string->number (string (string-ref str inicio))) (comblis str (+ inicio 2)) )]
                        )
 ))

(define octal->list (lambda (str)
                     (comblis (retirar-base str) 0 )
                     ))
